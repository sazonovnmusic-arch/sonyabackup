...
### Multi-profile gateway issues

When running multiple Hermes profiles (each with its own Telegram bot), gateway startup can fail with `Telegram bot token already in use (PID <N>)` or `already running under systemd`. The root cause is usually **stale lock files** left behind by a crashed/killed gateway process.

**Hermes uses machine-scoped locks** to prevent the same Telegram bot token from being used by two gateways simultaneously. Locks live in `~/.local/state/hermes/gateway-locks/` (keyed by SHA256 of the token). Each profile also writes `gateway.pid` + `gateway.lock` inside its profile dir.
...
# Check stale machine locks
ls -la ~/.local/state/hermes/gateway-locks/
cat ~/.local/state/hermes/gateway-locks/*.lock

# Check profile-level locks
ls ~/.hermes/profiles/*/gateway.lock ~/.hermes/gateway.lock
```

#### Fix stale locks after hard kill

If a gateway was killed with `kill -9` or crashed, its lock files survive and block restarts:
...
#### Fix "already running under systemd" for non-systemd profiles

If you start a profile manually while any gateway is running under systemd, Hermes detects the systemd unit and refuses to start another instance. Solutions:
- **Option A:** Use `hermes gateway run --force` to override (risky — orphan dispatcher)
- **Option B:** Create a dedicated systemd service per profile (recommended for production)
- **Option C:** Stop the main systemd service, run all profiles manually, then restart the main one

#### Token uniqueness

Each profile **must** use a distinct Telegram bot token. If two profiles share the same token, only one can connect. Check with:

```bash
for p in ~/.hermes/profiles/*/.env ~/.hermes/.env; do
    echo "$(dirname "$p"): $(grep TELEGRAM_BOT_TOKEN "$p" | sed 's/TELEGRAM_BOT_TOKEN=***')"
done
```

If `traffic` and `main` share a token, create a new bot via @BotFather for one of them.

Full incident walkthrough and log patterns: `references/multi-profile-gateway-incident.md`
