...
========================================================
Пул proxy с round-robin ротацией для youtube-transcript-api и yt-dlp.

Поддерживает любые форматы. Автоопределение типа из URL.
...
Session Rotation (GonzoProxy-style):
  Меняем `sess-XXXX` в логине → новый IP.
  Поддерживает: sess-, session-, sp, rnd, sid, sessid

Установка proxy:
  RESIDENTIAL_PROXY_URL       = один proxy
  RESIDENTIAL_PROXY_URLS      = несколько через запятую
  SOCKS5_PROXY_URLS           = SOCKS5 через запятую
  PROXY_1, PROXY_2, ...       = нумерованные

Установка ротации:
  PROXY_STRATEGY              = round_robin | random | least_failures
  PROXY_MIN_INTERVAL          = минимум секунд между запросами с одного IP
"""
import os
import time
import random
...
class ProxyRotator:
    """Универсальный rotator для любых proxy."""
    ...
    def _load_from_env(self):
        ...
        if self.proxies:
            ...
            print("[ProxyRotator] No proxies configured")
