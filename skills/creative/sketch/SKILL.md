...
## Special pattern: Multi-agent team dashboard

When the user wants a UI to manage multiple AI agents (profiles) with chat + kanban, use this sub-pattern inside sketch:

1. **Agent list with emoji avatars** — left sidebar or top bar, one card/row per agent, colored circle avatar with emoji (e.g. 🧠 Default, 💻 Coding, 🎬 YouTube). Shows name + role + online status dot.
2. **Kanban board** — columns: Todo / In Progress / Done. Each card tagged with agent color. Drag/drop optional (visual only for sketch).
3. **Chat panel** — either bottom dock (Teamly-style) or full tab (Dashboard-style). Shows selected agent's avatar next to messages.
4. **Dark theme by default** — agent dashboards look better on dark backgrounds. Use slate/indigo/purple palette.

**Two proven layout stances:**
- **"Teamly" style** — agents sidebar + kanban center + chat bottom panel. Everything visible at once.
- **"Dashboard" style** — agents sidebar + tabbed main area (Chat / Tasks / Logs). More space per tab.

See `references/multi-agent-dashboard-examples.md` for ready-made HTML templates.

### Retro / Pixel Art Aesthetic

When the user wants a playful, game-like interface (e.g., "pixel office", "8-bit characters", "retro game style"), use CSS box-shadow pixel art instead of images. This keeps sketches lightweight, interactive, and free of external assets.

**Technique: CSS box-shadow pixel sprites**
Each "pixel" is a 4px×4px `box-shadow` offset. No image files, no build step.
...
.pixel-body {
  ...
  box-shadow:
    ...
    /* body */
    4px 16px 0 #1e3a8a, 8px 16px 0 #2563eb, ...;
}
```

**Typical sprite sizes:**
- Head: 8×8 to 12×12 pixels
- Body: 8×12 to 12×16 pixels
- Total character height: ~40–50px
- Use `image-rendering: pixelated` on container

**Office scene composition:**
- Dark background (`#1a1a2e` or `#0f172a`)
- Simple divs for floor, window, whiteboard, plant, coffee machine
- Desks arranged in cross or grid pattern
- Characters positioned absolutely over desks with `idle` animation (subtle bounce)
- Name tags below characters

**Color palettes per agent role:**
| Role | Primary | Dark | Light |
|------|---------|------|-------|
| Leader / Default | `#3b82f6` | `#1e3a8a` | `#60a5fa` |
| Coding | `#7c3aed` | `#4c1d95` | `#a78bfa` |
| YouTube / Content | `#dc2626` | `#7f1d1d` | `#f87171` |
| Traffic / Ads | `#ea580c` | `#9a3412` | `#fb923c` |
| SMM / Insta | `#ec4899` | `#9d174d` | `#f9a8d4` |

Full sprite templates and copy-paste ready HTML are in `references/pixel-art-css-technique.md`.

### Evaluating third-party tools for integration (quick check)

When the user finds a SaaS (like Teamly, Lobe, etc.) and asks "can we plug our agent in?", check these URLs in order before declaring it viable:
1. `https://<domain>/api` — public API docs?
2. `https://<domain>/docs` — documentation?
3. Pricing page — does it mention custom agents / API?
4. `/pricing` — free tier for self-hosted agents?

If `/api` redirects to login and `/docs` 404s → **closed SaaS, not integrable**. State this clearly with alternatives.
