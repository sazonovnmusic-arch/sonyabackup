...
When configuring custom providers (OpenAI‑compatible APIs) and encountering `APIConnectionError`, `AuthenticationError`, or `HTTP 404` errors, follow this diagnostic sequence:

1. **DNS/network check**:
   ```bash
   curl -I https://your-endpoint.com/v1 --connect-timeout 5
   dig your-endpoint.com +short
   ping -c 2 your-endpoint.com
   ```
   If DNS returns NXDOMAIN (non‑existent domain) or ping fails, the endpoint URL is wrong or the service is unreachable.

2. **Validate API endpoint structure**:
   Many multi‑model gateways (like OpenModel.ai) require specific path prefixes. Test common patterns:
   ```bash
   # Try these variations if /v1/chat/completions returns 404
   curl https://api.openmodel.ai/v1/chat/completions
   curl https://api.openmodel.ai/openai/v1/chat/completions
   curl https://api.openmodel.ai/api/v1/chat/completions
   ```
   Check the provider's documentation for exact path requirements.

3. **Test authentication with simple GET**:
   ```bash
   curl -H "Authorization: Bearer YOUR_KEY" https://your-endpoint.com/v1/models
   ```
   If `/models` returns models list but `/chat/completions` returns 404, the endpoint supports listing but not completion — contact the provider.

4. **Test small completion** (to verify model access):
   ```bash
...
5. **OpenModel.ai specific note**: As of June 2026, `api.openmodel.app` does not exist (NXDOMAIN) and `api.openmodel.ai` returns `{"success":false,"error":{"code":"NOT_FOUND","msg":"route not found"}}` for all standard OpenAI paths. If you have an OpenModel.ai key, check their dashboard for actual endpoint and path conventions.

6. **Config fix for common errors**:
   - `APIConnectionError` / NXDOMAIN → wrong base_url
   - `HTTP 401` → invalid or missing api_key
   - `HTTP 404` → incorrect endpoint path (add `/openai/v1/` prefix)
   - `HTTP 429` → rate limited — switch to a different key in pool

   Use `hermes config set model.base_url` and `hermes config set model.provider` accordingly.

7. **Fallback to known working providers**:
   If the custom endpoint fails consistently, switch to a supported built‑in provider:
   ```bash
   ...
   ```
   Ollama Cloud supports DeepSeek v3.2 and v4‑flash with `OLLAMA_API_KEY`.
