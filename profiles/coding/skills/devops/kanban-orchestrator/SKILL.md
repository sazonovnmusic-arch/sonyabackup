...
The orchestrator profile handles every user message, so its model choice affects latency and cost disproportionately. Consider:

- **Reasoning + tool-use quality over raw knowledge depth.** The orchestrator plans and routes; workers execute. A model with excellent function calling (e.g., GPT-4o, Gemini Flash) often outperforms a heavier reasoning model (e.g., DeepSeek-V4, Kimi K2.6) for this role.
- **Vision capability only if the orchestrator processes images directly** (e.g., analyzing UI screenshots or user-uploaded photos). If vision lives only in a sub-agent, the orchestrator does not need it.
- **Cost matters at scale.** An orchestrator running 50+ times/day on an expensive model adds up. Cheaper models with good tool use are often sufficient.
- **Provider catalogs differ.** On Ollama Cloud, vision-capable models include the Gemma 3/4 family, Gemini Flash, and Kimi K2.x series; DeepSeek and Mistral variants are typically text-only. Always check the provider's model list before assuming vision support.

If no existing profile fits a required specialty, ask the user which to create or use rather than inventing a name the dispatcher will silently drop.
