...
### `.env` credential masking — write with Python, NOT terminal tools

Hermes **masks credential values** in terminal output, `read_file`, and even `write_file`/`patch` tool calls. When you write a value like `TELEGRAM_ALLOWED_USERS=1344036571` via `write_file`, it can silently become `TELEGRAM_ALLOWED_USERS=***` in the actual file. This causes hard-to-debug failures (bot rejects all users, API key 400 errors, etc.).

**Safe pattern for writing credentials to `.env`:**
```python
# Use Python open() inside execute_code or terminal — bypasses masking
python3 -c "
with open('/path/to/.env', 'w') as f:
    f.write('TELEGRAM_BOT_TOKEN=123:abc\n')
    f.write('TELEGRAM_ALLOWED_USERS=1344036571\n')
"
```
Then **verify with raw bytes** (not `cat`/`grep`, which also mask):
```python
python3 -c "
...
**Never** use `write_file`, `patch`, `cat`, or `grep` to write or verify credentials — they will mask the values and you won't notice until the service fails.

### Default profile `.env` location

The **default** (unnamed) profile uses `HERMES_HOME=/root/.hermes` — its `.env` is at `/root/.hermes/.env`, NOT `/root/.hermes/profiles/default/.env`. Named profiles (youtube, coding, traffic) use `~/.hermes/profiles/<name>/.env`. If a gateway service shows "Unauthorized user" or "No user allowlists configured" despite credentials being set, check that you wrote to the `.env` that matches the service's `HERMES_HOME` env var:
```bash
cat /proc/$(pgrep -f "hermes_cli.main gateway" | head -1)/environ | tr '\0' '\n' | grep HERMES_HOME
```
